...
class BooksDetails extends PostgresConnector{
  ...
  def read(stm: Statement): List[Books] = {
    val sqlRead = "SELECT * from books_details"
  ...
  def update(stm: Statement, set1: Map[String, Any], condition1: Map[String, Any]): Int = {
    val sqlUpdate = s"UPDATE books_details SET ${set1.map(p => s"${p._1}='${p._2}'").mkString(" , ")} WHERE ${condition1.map(p => s"${p._1}='${p._2}'").mkString(" AND ")}"
  stm.executeUpdate(sqlUpdate)
  }

  def delete(stm: Statement, condition2: Map[String, Any]): Int = {
    val sqlDelete = s"DELETE FROM books_details WHERE ${condition2.map(p => s"${p._1}='${p._2}'").mkString(" AND ")}"
